...
// ========== 同步功能 - LocalStorage本地缓存 ==========
// 1. 定义本地存储密钥（自定义名称，避免与其他缓存冲突）
const SYNC_STORAGE_KEY = 'attendance_sync_data'; // 修改为考勤系统的同步键名
// 2. 获取你要同步的DOM元素（关键！替换为你自己的元素选择器，如表单、文本框等）
// 暂时设置为null，因为考勤系统可能不需要特定的同步载体元素
const syncTarget = null;

// 3. 页面加载时，从LocalStorage恢复数据到你的同步载体
// 这里暂时注释掉具体的恢复逻辑，因为需要根据实际的同步载体类型来实现
// if (localStorage.getItem(SYNC_STORAGE_KEY) && syncTarget) {
//     // 根据你的元素类型修改赋值方式（如input/textarea用value，div用innerHTML）
//     syncTarget.value = localStorage.getItem(SYNC_STORAGE_KEY);
//     // 若你的同步载体是多个表单元素，可存储JSON格式数据，此处对应解析即可
//     // 示例：const syncData = JSON.parse(localStorage.getItem(SYNC_STORAGE_KEY));
//     // 示例：document.getElementById('name').value = syncData.name;
// }

// 4. 辅助函数：更新同步状态（若你新增了status元素，可使用；无则删除此函数）
...
// ========== 同步功能 - PeerJS（WebRTC）初始化（插入到你的JS文件中，LocalStorage之后） ========== 
 // 1. 初始化PeerJS实例（免费公共信令服务，无需自建服务器） 
 const peer = new Peer(); 
 let syncConn = null; // 存储点对点连接实例 
 
 // 2. 获取连接相关元素（若你新增了输入框和按钮，否则可删除此部分） 
 const myPeerIdEl = document.getElementById('myPeerId'); 
 const targetPeerIdEl = document.getElementById('targetPeerId'); 
 const connectBtn = document.getElementById('connectBtn'); 
 
 // 3. 监听PeerJS初始化成功，获取并展示设备ID 
 peer.on('open', (peerId) => { 
     if (myPeerIdEl) { 
         myPeerIdEl.textContent = peerId; 
         updateSyncStatus(`当前设备ID：<span class="peer-id">${peerId}</span>（已就绪）`); 
     } 
     console.log('设备ID生成成功：', peerId); 
 }); 
 
 // 4. 监听外来连接请求（被动接收其他设备连接） 
 peer.on('connection', (incomingConn) => { 
     if (syncConn) { 
         incomingConn.close(); 
         alert('已存在同步连接，请勿重复连接'); 
         return; 
     } 
     syncConn = incomingConn; 
     initSyncConnection(syncConn); // 初始化连接事件 
     updateSyncStatus('已接收连接，开始实时同步'); 
 }); 
 
 // 5. 主动建立连接（绑定按钮点击事件，若你未新增按钮，可删除此部分，通过控制台调用） 
 if (connectBtn) { 
     connectBtn.addEventListener('click', () => { 
         const targetId = targetPeerIdEl ? targetPeerIdEl.value.trim() : ''; 
         if (!targetId) { 
             alert('请输入对方设备ID'); 
             return; 
         } 
         ...
         syncConn = peer.connect(targetId); 
         initSyncConnection(syncConn); 
         updateSyncStatus(`正在连接设备【${targetId}】...`); 
     }); 
 } 
 
 // 6. PeerJS异常处理 
 peer.on('error', (err) => { 
     updateSyncStatus(`同步错误：${err.message}（请刷新页面重试）`); 
     console.error('PeerJS异常：', err); 
...
// ========== 同步功能 - 数据收发与实时同步（插入到你的JS文件中，PeerJS之后） ========== 
 // 1. 初始化连接事件（数据收发核心） 
 function initSyncConnection(connection) { 
     // 连接成功：立即发送本地最新数据 
     connection.on('open', () => { 
         updateSyncStatus(`连接成功！已开启实时同步`); 
         sendSyncData(); // 发送本地现有数据 
     }); 

     // 接收对方数据：更新本地载体 + 本地缓存 
     connection.on('data', (receiveData) => { 
         if (receiveData.content !== undefined && syncTarget) { 
             // 根据你的同步载体类型修改赋值方式（与LocalStorage恢复时一致） 
             syncTarget.value = receiveData.content; 
             // 若同步多个表单元素，此处解析JSON并赋值 
             // const syncData = receiveData.content; 
             // document.getElementById('name').value = syncData.name; 
             
             // 同步更新本地缓存 
             localStorage.setItem(SYNC_STORAGE_KEY, receiveData.content); 
             // 若为JSON格式：localStorage.setItem(SYNC_STORAGE_KEY, JSON.stringify(syncData)); 
         } 
     }); 

     // 连接断开处理 
     connection.on('close', () => { 
         updateSyncStatus(`连接已断开，可重新建立连接`); 
         syncConn = null; 
     }); 

     // 连接错误处理 
     connection.on('error', (err) => { 
         updateSyncStatus(`连接失败：${err.message}`); 
         syncConn = null; 
     }); 
 } 
 ...
 function sendSyncData() { 
     if (syncConn && syncConn.open && syncTarget) { 
         // 组装同步数据：根据你的需求修改（单个元素用value，多个元素用JSON） 
         const syncContent = syncTarget.value; // 单个元素 
         // 若为多个表单元素： 
         // const syncContent = { 
         ...
         syncConn.send({ 
             content: syncContent, 
             timestamp: new Date().getTime() // 用于解决数据冲突 
         }); 
     } 
 } 

 // 3. 监听你的同步载体变化，实时触发同步（关键！） 
 if (syncTarget) { 
     // 根据你的元素类型选择事件（input适用于输入框，change适用于下拉框等） 
     syncTarget.addEventListener('input', () => { 
         // 先更新本地缓存 
         const currentContent = syncTarget.value; 
         localStorage.setItem(SYNC_STORAGE_KEY, currentContent); 
         // 再发送给对方设备 
         sendSyncData(); 

         // 若为多个表单元素，监听每个表单的change/input事件，组装JSON后缓存+发送 
         // const syncData = { 
         //     name: document.getElementById('name').value, 
         //     phone: document.getElementById('phone').value 
         // }; 
...
function initData() {
    try {
        console.log('开始执行initData函数...');
        
        // 确保localStorage中的所有必要数据都存在，至少初始化为空数组或空对象
        const requiredKeys = {
            'members': '[]',
            'groups': '[]',
            'users': '{}',
        ...
        Object.entries(requiredKeys).forEach(([key, defaultValue]) => {
            if (!localStorage.getItem(key)) {
                ...
                console.log(`  ✓ 初始化localStorage.${key}为默认值: ${defaultValue}`);
            } else {
                console.log(`  ✓ localStorage.${key}已存在`);
            }
        });
        ...
        if (!users['赵淮']) {
            ...
            console.log('  ✓ 添加新负责人用户：赵淮');
        } else {
            console.log('  ✓ 负责人用户赵淮已存在');
        }
        
        console.log('✓ 初始化数据函数执行完成');
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('====================================');
    console.log('DOMContentLoaded事件被触发，页面加载完成！');
    console.log('====================================');
    
    try {
        console.log('2. 开始初始化数据...');
        initData();
        console.log('   ✓ 数据初始化完成');
        
        console.log('3. 开始设置事件监听器...');
        setupEventListeners();
        console.log('   ✓ 事件监听器设置完成');
        
        // 手动调用一次togglePasswordField，确保初始状态正确
        console.log('4. 初始化密码字段显示状态...');
        togglePasswordField();
        console.log('   ✓ 密码字段状态已初始化');
        
        // 设置默认日期
        console.log('5. 设置默认日期...');
        ...
        if (startDate) startDate.value = new Date(new Date().setMonth(new Date().getMonth() - 1)).toISOString().split('T')[0];
        if (endDate) endDate.value = today;
        console.log('   ✓ 默认日期设置完成');
        
        // 检查登录状态
        console.log('6. 检查登录状态...');
        const loginState = localStorage.getItem('loginState');
        if (loginState) {
            console.log('   ✗ 发现已有登录状态，准备显示主页面');
...
function setupEventListeners() {
    ...
    if (roleSelect) {
        ...
        console.log('角色选择事件监听器已绑定');
    } else {
        console.error('角色选择元素不存在');
    }
    ...
    if (logoutBtn) {
        ...
        console.error('退出按钮元素不存在');
    }
    
    console.log('导航按钮事件监听...');
    ...
    if (cancelMemberBtn) cancelMemberBtn.addEventListener('click', hideAddMemberForm);
    ...
    if (deleteAllMembersBtn) deleteAllMembersBtn.addEventListener('click', deleteAllMembers);
    
    // 考勤相关事件
    const attendanceDate = document.getElementById('attendance-date');
    if (attendanceDate) attendanceDate.addEventListener('change', loadAttendanceData);
    ...
    if (searchMembersInput) searchMembersInput.addEventListener('input', searchMembers);
    ...
    if (addLeaderBtn) addLeaderBtn.addEventListener('click', showAddLeaderForm);
    
    const cancelLeaderBtn = document.getElementById('cancel-leader-btn');
    if (cancelLeaderBtn) cancelLeaderBtn.addEventListener('click', hideAddLeaderForm);
    
    const saveLeaderBtn = document.getElementById('save-leader-btn');
    if (saveLeaderBtn) saveLeaderBtn.addEventListener('click', saveLeader);
    
    const cancelEditLeaderBtn = document.getElementById('cancel-edit-leader-btn');
    if (cancelEditLeaderBtn) cancelEditLeaderBtn.addEventListener('click', hideEditLeaderForm);
    
    const saveEditLeaderBtn = document.getElementById('save-edit-leader-btn');
    if (saveEditLeaderBtn) saveEditLeaderBtn.addEventListener('click', saveEditLeader);
    
    // 分组管理事件
    const addGroupBtn = document.getElementById('add-group-btn');
    if (addGroupBtn) addGroupBtn.addEventListener('click', showAddGroupForm);
    
    const cancelGroupBtn = document.getElementById('cancel-group-btn');
    if (cancelGroupBtn) cancelGroupBtn.addEventListener('click', hideAddGroupForm);
    
    const saveGroupBtn = document.getElementById('save-group-btn');
    if (saveGroupBtn) saveGroupBtn.addEventListener('click', saveGroup);
    
    const deleteGroupBtn = document.getElementById('delete-group-btn');
    if (deleteGroupBtn) deleteGroupBtn.addEventListener('click', showDeleteGroupForm);
    
    const deleteAllGroupsBtn = document.getElementById('delete-all-groups-btn');
    if (deleteAllGroupsBtn) deleteAllGroupsBtn.addEventListener('click', deleteAllGroups);
    
    // 密码修改事件
    const changePasswordBtn = document.getElementById('change-password-btn');
    if (changePasswordBtn) changePasswordBtn.addEventListener('click', showChangePasswordForm);
    
    const cancelPasswordBtn = document.getElementById('cancel-password-btn');
    if (cancelPasswordBtn) cancelPasswordBtn.addEventListener('click', hideChangePasswordForm);
...
function deleteGroup() {
    ...
    try {
        ...
        // 检查是否还有其他分组使用相同的楼层
        const otherGroups = groups.filter(group => group.floor === groupFloor && group.name !== groupName);
...
function updateGroupLeaderDropdown() {
    try {
        ...
        const leaders = Object.keys(users)
            .filter(username => users[username].role === 'leader')
            .map(username => ({ username, name: users[username].name }));
...
function togglePasswordField() {
    console.log('togglePasswordField函数被调用');
    
    const role = document.getElementById('role').value;
    const passwordGroup = document.getElementById('password-group');
    
    console.log('当前角色:', role);
    console.log('密码字段元素:', passwordGroup);
...
function handleLogin() {
    ...
    console.log('元素检查:', {
        roleSelect: roleSelect ? '存在' : '不存在',
        usernameInput: usernameInput ? '存在' : '不存在',
        passwordInput: passwordInput ? '存在' : '不存在',
        errorDiv: errorDiv ? '存在' : '不存在'
    });
    ...
    if (role !== 'member' && !password) {
        console.log('密码为空且角色不是成员，显示错误信息');
        if (errorDiv) {
    ...
    try {
        ...
        if (role === 'member') {
            ...
            console.log('成员查询结果:', { member });
            
            if (!member) {
                console.log('成员验证失败: 成员不存在');
                if (errorDiv) {
            ...
            console.log('成员验证成功:', { currentUser, currentRole, currentMemberId });
        } else {
            console.log('验证管理员/负责人登录...');
            // 从localStorage获取用户信息
            const users = JSON.parse(localStorage.getItem('users') || '{}');
            const user = users[username];
            
            console.log('管理员/负责人查询结果:', { user });
            
            if (!user || user.password !== password || user.role !== role) {
                console.log('管理员/负责人验证失败');
                if (errorDiv) {
            ...
            currentUser = username;
            currentRole = role;
            console.log('管理员/负责人验证成功:', { currentUser, currentRole });
        }
        
        // 保存登录状态
        console.log('保存登录状态到localStorage...');
...
function switchPage(pageName) {
    ...
    if (pageName === 'dashboard') {
        loadDashboardData();
    } else if (pageName === 'member-info' && currentRole === 'member') {
        loadMemberInfo();
...
function updateAttendanceForNewMember(member) {
    try {
        ...
        // 如果记录不存在，创建新记录
        if (!existingRecord) {
            const newRecord = { member_id: member.id, date: today, status: 'absent' };
...
function updateMemberFloor(memberId, floor) {
    ...
    try {
        ...
        const updatedMembers = members.map(member => 
            member.id === memberId ? { ...member, floor: floor || '' } : member
        );
        
        // 保存更新后的成员数据回localStorage
...
function deleteMember(memberId) {
    ...
    if (confirm('确定要删除该成员吗？')) {
        try {
            ...
            // 从localStorage中的签到记录中删除
            const attendance = JSON.parse(localStorage.getItem('attendance') || '[]');
            const updatedAttendance = attendance.filter(record => record.member_id !== memberId);
...
function loadAttendanceData() {
    try {
        ...
        if (!attendanceDateElement) {
            console.error('未找到attendance-date元素，无法加载签到数据');
            return;
        }
        
        if (!attendanceListElement) {
            console.error('未找到attendance-list元素，无法加载签到数据');
            return;
        }
        
        const date = attendanceDateElement.value;
        const tbody = attendanceListElement;
        ...
        console.log('成员数据字符串:', membersStr);
        let members = [];
        try {
            members = JSON.parse(membersStr || '[]');
            // 确保members是一个数组
            if (!Array.isArray(members)) {
                console.error('成员数据不是数组，已重置为空数组');
                members = [];
            }
        } catch (error) {
            console.error('解析成员数据失败，已重置为空数组:', error);
            members = [];
        }
        console.log('解析后的成员数据:', members);
        
        // 从localStorage获取分组数据
        const groupsStr = localStorage.getItem('groups');
        console.log('分组数据字符串:', groupsStr);
        let groups = [];
        try {
            groups = JSON.parse(groupsStr || '[]');
            // 确保groups是一个数组
            if (!Array.isArray(groups)) {
                console.error('分组数据不是数组，已重置为空数组');
                groups = [];
            }
        } catch (error) {
            console.error('解析分组数据失败，已重置为空数组:', error);
            groups = [];
        }
        console.log('解析后的分组数据:', groups);
        
        // 从localStorage获取用户数据
        const usersStr = localStorage.getItem('users');
        console.log('用户数据字符串:', usersStr);
        let usersObj = {};
        try {
            usersObj = JSON.parse(usersStr || '{}');
            // 确保usersObj是一个对象
            if (typeof usersObj !== 'object' || usersObj === null || Array.isArray(usersObj)) {
                console.error('用户数据不是对象，已重置为空对象');
                usersObj = {};
            }
        } catch (error) {
            console.error('解析用户数据失败，已重置为空对象:', error);
            usersObj = {};
        }
        console.log('解析后的用户数据:', usersObj);
        
        // 从localStorage获取签到数据
        const attendanceStr = localStorage.getItem('attendance');
        console.log('签到数据字符串:', attendanceStr);
        let attendanceRecords = [];
        try {
            attendanceRecords = JSON.parse(attendanceStr || '[]');
            // 确保attendanceRecords是一个数组
            if (!Array.isArray(attendanceRecords)) {
                console.error('签到数据不是数组，已重置为空数组');
                attendanceRecords = [];
            }
        } catch (error) {
            console.error('解析签到数据失败，已重置为空数组:', error);
            attendanceRecords = [];
        }
        console.log('解析后的签到数据:', attendanceRecords);
        
        // 将签到记录转换为与原代码兼容的格式
        const attendanceMap = {};
        try {
            attendanceRecords.forEach(record => {
                if (record && record.date === date && record.member_id) {
                    attendanceMap[record.member_id] = record.status;
                }
            });
        } catch (error) {
            console.error('处理签到记录时出错:', error);
        }
        console.log('签到数据映射:', attendanceMap);
        
        // 过滤数据：如果是负责人，只显示自己负责的分组
        let filteredMembers = members;
        if (currentRole === 'leader') {
            const user = usersObj[currentUser];
            if (user) {  // 添加对user对象存在性的检查
                const managedFloors = user.managedFloors || [];
                const leaderGroups = groups.filter(group => managedFloors.includes(group.floor));
                const groupNames = leaderGroups.map(group => group.name);
                filteredMembers = members.filter(member => member && member.group && groupNames.includes(member.group));
            }
            // 如果user不存在，默认显示所有成员
        } else {
            // 非负责人角色：确保只处理有效的成员数据
            filteredMembers = members.filter(member => member && member.id);
        }
...
function updateAttendance(memberId, status, date) {
    try {
        ...
        const existingIndex = attendanceRecords.findIndex(record => 
            record.member_id === memberId && record.date === date
        );
        ...
        if (existingIndex !== -1) {
            ...
            // 创建新记录
            attendanceRecords.push({ member_id: memberId, date, status });
        }
...
function updateMemberHours(memberId) {
    try {
        ...
        // 更新成员课时
        const updatedMembers = members.map(member => 
            member.id === memberId ? { ...member, hours } : member
        );
...
function calculateMemberHours(memberId) {
    try {
        // 验证memberId参数
        if (!memberId) {
            console.error('计算成员课时时出错: memberId为空');
            return 0;
        }
        
        // 从localStorage获取所有考勤记录
        const attendanceStr = localStorage.getItem('attendance');
        let attendanceRecords = [];
        try {
            attendanceRecords = JSON.parse(attendanceStr || '[]');
            // 确保attendanceRecords是数组
            if (!Array.isArray(attendanceRecords)) {
                console.error('计算成员课时时出错: 签到记录不是数组');
                attendanceRecords = [];
            }
        } catch (error) {
            console.error('解析签到记录时出错:', error);
            attendanceRecords = [];
        }
        
        // 过滤出该成员的所有签到记录
        const presentRecords = attendanceRecords.filter(record => 
            record && record.member_id === memberId && record.status === 'present'
        );
...
function loadDashboardData() {
    ...
    try {
        ...
        const todayLeaveElement = document.getElementById('today-leave');
        const todayAbsentElement = document.getElementById('today-absent');
        const totalHoursElement = document.getElementById('total-hours');
        
        if (!todayAttendanceElement) {
            console.error('未找到today-attendance元素，无法加载仪表盘数据');
            return;
        }
        
        if (!todayLeaveElement) {
            console.error('未找到today-leave元素，无法加载仪表盘数据');
            return;
        }
        
        if (!todayAbsentElement) {
            console.error('未找到today-absent元素，无法加载仪表盘数据');
            return;
        }
        
        if (!totalHoursElement) {
            console.error('未找到total-hours元素，无法加载仪表盘数据');
            return;
        }
        ...
        console.log('解析后的签到数据:', attendanceRecords);
        const todayAttendanceRecords = attendanceRecords.filter(record => record && record.date === today);
        console.log('今日签到记录:', todayAttendanceRecords);
        ...
        console.log('今日签到数据映射:', todayAttendance);
        ...
        filteredMembers.forEach(member => {
            try {
                ...
                console.error('处理成员签到状态时出错:', error);
            }
        });
        ...
        todayAttendanceElement.textContent = present;
        todayLeaveElement.textContent = leave;
        todayAbsentElement.textContent = absent;
...
function loadMemberInfo() {
    ...
    try {
        ...
        if (member) {
            ...
            // 从localStorage获取该成员的所有签到记录
            const attendanceRecords = JSON.parse(localStorage.getItem('attendance') || '[]');
            const memberAttendanceRecords = attendanceRecords.filter(record => record.member_id === currentMemberId);
...
function extractFloorFromGroupName(groupName) {
    // 尝试匹配常见的楼层模式，如"一楼", "二楼", "1楼", "2楼", "地下一楼", "B1楼"
    const patterns = [
...
function parseExcel(file) {
    ...
    reader.onload = function(e) {
        ...
        const possibleHeaders = ['姓名', '名字', '名称', '学生', '成员', '学号', 'ID', '编号', '工号', '楼层', '楼', '层', '分组', '组', '团队', '签到状况'];
        let headerCount = 0;
            ...
            for (let C = 0; C < Math.min(4, range.e.c + 1); C++) {
                ...
                if (cell && cell.v) {
                    ...
                    if (expectedHeader === '姓名' && !(header.includes('姓名') || header.includes('名字') || header.includes('名称') || header.includes('学生') || header.includes('成员'))) {
                        ...
                        isSpecificFormat = false;
                        break;
                    } else if (expectedHeader === '楼层' && !(header.includes('楼层') || header.includes('楼') || header.includes('层'))) {
                        isSpecificFormat = false;
                        break;
            ...
            if (isSpecificFormat) {
                ...
                for (let C = 0; C <= range.e.c; C++) {
                    ...
                    if (cell && cell.v) {
                        ...
                        if (header.includes('楼层') || header.includes('楼') || header.includes('层')) {
                            headers['楼层'] = C;
                        }
...
function exportData() {
    ...
    try {
        ...
        // 从localStorage获取签到数据
        let allAttendanceRecords = JSON.parse(localStorage.getItem('attendance') || '[]');
...
function generateSummary() {
    ...
    try {
        ...
        if (filterFloor) {
            // 先获取该楼层的所有分组            const floorGroups = groups.filter(group => group.floor === filterFloor).map(group => group.name);
            // 过滤成员：属于该楼层的分组            members = members.filter(member => member.group && floorGroups.includes(member.group));
        }
        ...
        // 从localStorage获取今日签到数据
        const allAttendanceRecords = JSON.parse(localStorage.getItem('attendance') || '[]');
        const todayAttendanceRecords = allAttendanceRecords.filter(record => record.date === today);
    ...
    attendanceData.forEach(item => {
        ...
        if (member) {
            ...
            const existingIndex = attendance.findIndex(record => 
                record.member_id === member.id && record.date === date
            );
            
            if (existingIndex !== -1) {
                // 更新已有记录
                attendance[existingIndex].status = normalizedStatus;
            } else {
                // 创建新记录
                attendance.push({ member_id: member.id, date: date, status: normalizedStatus });
            }
