...
function renderImportPage() {
    ...
    // 更新成员列表的分组筛选
    const membersGroupFilter = document.getElementById('membersGroupFilter');
    membersGroupFilter.innerHTML = '<option value="">全部分组</option>' + 
        groups.map(group => `<option value="${group.name}">${group.name}</option>`).join('');
...
function renderMembersList() {
    ...
    const selectedGroup = document.getElementById('membersGroupFilter').value;
...
function deleteMember(memberId) {
    if (confirm('确定要删除该成员吗？删除后该成员的所有签到记录也将被删除。')) {
        // 删除成员
        members = members.filter(member => member.id !== memberId);
        
        // 删除该成员的所有签到记录
        signinRecords = signinRecords.filter(record => record.memberId !== memberId);
