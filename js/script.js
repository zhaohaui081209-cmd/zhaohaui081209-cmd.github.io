...
// 在window对象上创建Supabase客户端实例，避免与全局变量冲突
window.appSupabase = window.createClient(supabaseUrl, supabaseKey);

// 初始化数据
async function initData() {
    try {
        // 检查默认管理员用户是否存在
        const { data: adminData, error: adminError } = await window.appSupabase
            .from('users')
            .select('*')
            .eq('username', '赵广仁')
            .single();
        ...
        // 如果管理员用户不存在，创建它
        if (!adminData) {
            const { error: insertAdminError } = await window.appSupabase
                .from('users')
                .insert([
        ...
        // 检查默认负责人用户是否存在
        const { data: leaderData, error: leaderError } = await window.appSupabase
            .from('users')
            .select('*')
            .eq('username', '负责人')
            .single();
        ...
        if (leaderError && leaderError.code !== 'PGRST116') {
            console.error('检查负责人用户时出�?', leaderError);
            return;
        }

        // 如果负责人用户不存在，创建它
        if (!leaderData) {
            const { error: insertLeaderError } = await window.appSupabase
                .from('users')
                .insert([
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    // 检查登录状�?    const loginState = localStorage.getItem('loginState');
    if (loginState) {
...
// 设置事件监听�?function setupEventListeners() {
    ...
    // 负责人管理事�?    document.getElementById('add-leader-btn').addEventListener('click', showAddLeaderForm);
...
// 显示添加负责人表�?function showAddLeaderForm() {
...
// 隐藏添加负责人表�?function hideAddLeaderForm() {
...
function generateFloorOptions(selectId = 'leader-floors') {
    ...
    // 获取所有唯一的楼�?    const floors = [...new Set(groups.map(group => group.floor).filter(Boolean))].sort();
...
// 显示编辑负责人表�?function showEditLeaderForm(username) {
...
// 隐藏编辑负责人表�?function hideEditLeaderForm() {
...
// 保存编辑后的负责人信�?function saveEditLeader() {
    ...
    // 检查用户名是否已存在（如果修改了用户名�?    if (username !== originalUsername && users[username]) {
        alert('该用户名已存在');
        return;
    }
    ...
    // 添加新的用户记录（可能使用新的用户名�?    users[username] = updatedUser;
    
    // 更新分组中的负责人引用（如果用户名发生了变化�?    if (username !== originalUsername) {
    ...
    // 更新分组表单中的负责人下拉列�?    updateGroupLeaderDropdown();
}

// 保存负责�?function saveLeader() {
...
// 删除负责�?function deleteLeader(username) {
...
// 加载负责人列�?function loadLeaders() {
    ...
    leaders.forEach(username => {
        ...
        // 找出该负责人管理的分�?        const managedGroups = groups.filter(group => managedFloors.includes(group.floor));
...
async function saveGroup() {
    // 检查权限：只有管理员才能添加分�?    if (currentRole !== 'admin') {
        alert('只有管理员才能添加分�?);
        return;
    }
    ...
    try {
        // 检查分组是否已存在
        const { data: existingGroup, error: checkError } = await window.appSupabase
            .from('groups')
            .select('*')
            .eq('name', name)
            .single();
        ...
        // 添加新分�?        const newGroup = { name, floor, leader: leader || '' };
        const { error: insertError } = await window.appSupabase
            .from('groups')
            .insert([newGroup]);
        
        if (insertError) {
            console.error('添加分组时出�?', insertError);
            alert('添加分组失败，请稍后重试');
            return;
        }
...
async function showDeleteGroupForm() {
    // 检查权限：只有管理员才能删除分�?    if (currentRole !== 'admin') {
        alert('只有管理员才能删除分组');
        return;
    }
    ...
    try {
        // 从Supabase获取分组列表
        const { data: groups, error: groupsError } = await window.appSupabase
            .from('groups')
            .select('*');
        
        if (groupsError) {
            console.error('获取分组列表时出�?', groupsError);
        ...
        // 如果表单不存在，创建�?        if (!deleteForm) {
            ...
            // 添加事件监听�?            document.getElementById('confirm-delete-group-btn').addEventListener('click', deleteGroup);
        ...
        console.error('显示删除分组表单时出�?', error);
...
async function deleteGroup() {
    ...
    if (!confirm(`确定要删除分�?${groupName}"吗？`)) {
        return;
    }
    
    try {
        // 获取要删除的分组信息
        const { data: groupToDelete, error: groupError } = await window.appSupabase
            .from('groups')
            .select('*')
            .eq('name', groupName)
            .single();
        ...
        // 检查是否还有其他分组使用相同的楼层
        const { data: otherGroups, error: otherGroupsError } = await window.appSupabase
            .from('groups')
            .select('*')
            .eq('floor', groupFloor)
            .neq('name', groupName);
        ...
        // 删除分组
        const { error: deleteGroupError } = await window.appSupabase
            .from('groups')
            .delete()
            .eq('name', groupName);
        
        if (deleteGroupError) {
            console.error('删除分组时出�?', deleteGroupError);
            alert('删除分组失败，请稍后重试');
            return;
        }
        
        // 更新成员数据，将属于该分组的成员设为未分�?        const { error: updateMembersError } = await window.appSupabase
            .from('members')
            .update({ group: '' })
            .eq('group', groupName);
        
        if (updateMembersError) {
            console.error('更新成员分组信息时出�?', updateMembersError);
            alert('删除分组失败，请稍后重试');
            return;
        }
        ...
        if (!hasOtherGroupWithSameFloor) {
            const { error: clearFloorError } = await window.appSupabase
                .from('members')
                .update({ floor: '' })
                .eq('floor', groupFloor);
            
            if (clearFloorError) {
                console.error('清除成员楼层信息时出�?', clearFloorError);
                // 这里不返回，因为主要的分组删除已经完�?            }
        }
        ...
        console.error('删除分组时出�?', error);
        alert('删除分组失败，请稍后重试');
    }
}

// 一键删除所有分�?async function deleteAllGroups() {
    if (!confirm("确定要删除所有分组吗？此操作不可恢复�?)) {
        return;
    }
    
    try {
        // 删除所有分�?        const { error: deleteGroupsError } = await window.appSupabase
            .from('groups')
            .delete();
        ...
        // 更新所有成员，设为未分组并清除楼层信息
        const { error: updateMembersError } = await window.appSupabase
            .from('members')
            .update({ group: '', floor: '' });
        
        if (updateMembersError) {
            console.error('更新成员信息时出�?', updateMembersError);
            alert('删除分组失败，请稍后重试');
            return;
        }
...
async function loadGroups() {
    try {
        ...
        // 更新分组表单中的负责人下拉列�?        await updateGroupLeaderDropdown();
    } catch (error) {
        console.error('加载分组时出�?', error);
    }
}
...
async function updateGroupDropdown() {
    try {
        ...
        // 保存当前选中的�?        const currentValue = select.value;
        
        // 清空下拉列表
        select.innerHTML = '<option value="">未分�?/option>';
        ...
        // 恢复当前选中的�?        select.value = currentValue;
    } catch (error) {
        console.error('更新分组下拉列表时出�?', error);
    }
}

// 更新分组表单中的负责人下拉列�?async function updateGroupLeaderDropdown() {
    try {
        // 从Supabase获取负责人列�?        const { data: leaders, error: leadersError } = await window.appSupabase
            .from('users')
            .select('*')
            .eq('role', 'leader');
...
async function handleLogin() {
    ...
    try {
        if (role === 'member') {
            // 查询成员信息
            const { data: members, error: memberError } = await window.appSupabase
                .from('members')
                .select('*')
                .eq('name', username)
                .single();
            ...
            // 查询管理员或负责人信�?            const { data: users, error: userError } = await window.appSupabase
                .from('users')
                .select('*')
        ...
        // 保存登录状�?        localStorage.setItem('loginState', JSON.stringify({ user: currentUser, role: currentRole, memberId: currentMemberId }));
        
        // 刷新页面
        window.location.reload();
    } catch (error) {
        console.error('登录时出�?', error);
        errorDiv.textContent = '登录时发生错误，请稍后重�?;
    }
}

// 显示主页�?async function showMainPage() {
    ...
    // 确保页面滚动到顶�?    window.scrollTo(0, 0);
}

// 检查角色可见�?function checkRoleVisibility() {
    if (currentRole === 'member') {
        ...
        // 管理员可以看到所有按�?        document.getElementById('add-member-btn').style.display = 'inline-block';
...
// 退出登�?function handleLogout() {
    ...
    // 清除登录状�?    localStorage.removeItem('loginState');
...
async function saveMember() {
    ...
    try {
        // 检查成员是否已存在
        const { data: existingMember, error: checkError } = await window.appSupabase
            .from('members')
            .select('*')
            .eq('id', id)
            .single();
        ...
        // 创建新成�?        const newMember = { id, name, hours: 0, group: group || '', floor: floor || '' };
        const { error: insertError } = await window.appSupabase
            .from('members')
            .insert([newMember]);
        
        if (insertError) {
            console.error('添加成员时出�?', insertError);
            alert('添加成员失败，请稍后重试');
            return;
        }
        ...
        // 添加到签到记�?        updateAttendanceForNewMember(newMember);
    } catch (error) {
        console.error('保存成员时出�?', error);
        alert('保存成员失败，请稍后重试');
    }
}
...
async function updateAttendanceForNewMember(member) {
    try {
        ...
        // 检查今天的签到记录是否已存�?        const { data: existingRecord, error: checkError } = await window.appSupabase
            .from('attendance')
            .select('*')
        ...
        // 如果记录不存在，创建新记�?        if (!existingRecord) {
            const { error: insertError } = await window.appSupabase
                .from('attendance')
                .insert([{ member_id: member.id, date: today, status: 'absent' }]);
            
            if (insertError) {
                console.error('为新成员添加签到记录时出�?', insertError);
            }
        }
    } catch (error) {
        console.error('为新成员更新签到记录时出�?', error);
    }
}
...
async function loadMembersData() {
    try {
        // 从Supabase加载成员数据
        const { data: members, error: membersError } = await window.appSupabase
            .from('members')
            .select('*');
        
        if (membersError) {
            console.error('加载成员数据时出�?', membersError);
            return;
        }
        ...
        if (groupsError) {
            console.error('加载分组数据时出�?', groupsError);
            return;
        }
        
        // 从Supabase加载用户数据
        const { data: users, error: usersError } = await window.appSupabase
            .from('users')
            .select('*');
        
        if (usersError) {
            console.error('加载用户数据时出�?', usersError);
            return;
        }
        ...
        // 应用筛�?        filteredMembers = applyFilters(filteredMembers, groups);
...
async function initFilters() {
    try {
        ...
        // 清空并添加楼层选项
        filterFloor.innerHTML = '<option value="">所有楼�?/option>';
        floors.forEach(floor => {
            filterFloor.appendChild(new Option(floor, floor));
        });
        
        // 初始化分组筛�?        await updateGroupFilter();
    } catch (error) {
        console.error('初始化筛选器时出�?', error);
    }
}

// 应用筛�?function applyFilters(members, groups) {
    const filterFloor = document.getElementById('filter-floor');
    ...
    // 应用楼层筛�?    const selectedFloor = filterFloor.value;
    if (selectedFloor) {
        filteredMembers = filteredMembers.filter(member => {
            // 获取成员的楼层信�?            let memberFloor = member.floor;
    ...
    // 应用分组筛�?    const selectedGroup = filterGroup.value;
    if (selectedGroup) {
...
async function updateGroupFilter() {
    try {
        ...
        // 清空分组选项
        filterGroup.innerHTML = '<option value="">所有分�?/option>';
        
        // 根据选择的楼层过滤分�?        let filteredGroups = groups;
        if (selectedFloor) {
        ...
        // 重新加载成员数据以应用筛�?        await loadMembersData();
    } catch (error) {
        console.error('更新分组筛选时出错:', error);
    }
}

// 清除筛�?async function clearFilters() {
...
// 清除统计页面筛�?async function clearStatisticsFilters() {
...
async function updateMemberFloor(memberId, floor) {
    ...
    try {
        const { error: updateError } = await window.appSupabase
            .from('members')
            .update({ floor: floor || '' })
            .eq('id', memberId);
        
        if (updateError) {
            console.error('更新成员楼层时出�?', updateError);
...
async function deleteMember(memberId) {
    // 检查权限：负责人不能删除成�?    if (currentRole === 'leader') {
        alert('负责人没有权限删除成�?);
        return;
    }
    ...
    if (confirm('确定要删除该成员吗？')) {
        try {
            // 从Supabase中删除成�?            const { error: deleteMemberError } = await window.appSupabase
                .from('members')
                .delete()
                .eq('id', memberId);
            
            if (deleteMemberError) {
                console.error('删除成员时出�?', deleteMemberError);
                alert('删除成员失败，请稍后重试');
                return;
            }
            
            // 从签到记录中删除
            const { error: deleteAttendanceError } = await window.appSupabase
                .from('attendance')
                .delete()
                .eq('member_id', memberId);
            
            if (deleteAttendanceError) {
                console.error('删除成员签到记录时出�?', deleteAttendanceError);
            }
            
            await loadMembersData();
            loadAttendanceData();
        } catch (error) {
            console.error('删除成员时出�?', error);
            alert('删除成员失败，请稍后重试');
        }
    }
}
...
async function loadAttendanceData() {
    try {
        ...
        // 从Supabase获取签到数据
        const { data: attendanceRecords, error: attendanceError } = await window.appSupabase
            .from('attendance')
            .select('*')
            .eq('date', date);
        
        if (attendanceError) {
            console.error('加载签到数据时出�?', attendanceError);
            return;
        }
        
        // 将签到记录转换为与原代码兼容的格�?        const attendanceMap = {};
        ...
        console.error('加载签到数据时出�?', error);
        alert('加载签到数据失败，请稍后重试');
    }
}

// 更新签到状�?async function updateAttendance(memberId, status, date) {
    try {
        // 检查签到记录是否已存在
        const { data: existingRecord, error: checkError } = await window.appSupabase
            .from('attendance')
            .select('*')
            .eq('member_id', memberId)
            .eq('date', date)
        ...
        // 插入或更新签到记�?        if (existingRecord) {
            ...
            if (updateError) {
                console.error('更新签到记录时出�?', updateError);
                return;
            }
        } else {
            // 创建新记�?            const { error: insertError } = await window.appSupabase
                .from('attendance')
                .insert([{ member_id: memberId, date, status }]);
            
            if (insertError) {
                console.error('添加签到记录时出�?', insertError);
                return;
            }
        }
        ...
        // 更新仪表盘数�?        if (date === new Date().toISOString().split('T')[0]) {
            await loadDashboardData();
        }
    } catch (error) {
...
// 重置签到状�?async function resetAttendance(memberId, date) {
...
async function updateMemberHours(memberId) {
    try {
        // 计算课时�?        const hours = await calculateMemberHours(memberId);
        ...
        if (updateError) {
            console.error('更新成员课时时出�?', updateError);
            return;
        }
    } catch (error) {
        console.error('更新成员课时时出�?', error);
    }
}
...
async function calculateMemberHours(memberId) {
    try {
        // 从Supabase获取该成员的所有签到记�?        const { data: attendanceRecords, error: attendanceError } = await window.appSupabase
            .from('attendance')
            .select('*')
            .eq('member_id', memberId)
            .eq('status', 'present');
        ...
        if (attendanceError) {
            console.error('计算成员课时时出�?', attendanceError);
            return 0;
        }
        ...
        console.error('计算成员课时时出�?', error);
        return 0;
    }
}

// 加载仪表盘数�?async function loadDashboardData() {
    const today = new Date().toISOString().split('T')[0];
    ...
    try {
        ...
        // 从Supabase获取今日签到数据
        const { data: todayAttendanceRecords, error: attendanceError } = await window.appSupabase
            .from('attendance')
            .select('*')
            .eq('date', today);
        ...
        // 将签到记录转换为与原代码兼容的格�?        const todayAttendance = {};
        ...
        // 计算总课�?        for (const member of filteredMembers) {
...
async function loadMemberInfo() {
    ...
    try {
        // 从Supabase获取成员数据
        const { data: member, error: memberError } = await window.appSupabase
            .from('members')
            .select('*')
        ...
        if (memberError) {
            console.error('加载成员数据时出�?', memberError);
            return;
        }
        ...
        if (member) {
            ...
            if (attendanceError) {
                console.error('加载签到记录时出�?', attendanceError);
                return;
            }
...
function setupFileUpload() {
    ...
    // 设置成员管理页面的文件上传区�?    ['dragenter', 'dragover', 'dragleave', 'drop'].forEach(eventName => {
...
function extractFloorFromGroupName(groupName) {
    // 尝试匹配常见的楼层模式，�?一�?, "二楼", "1�?, "2�?, "地下一�?, "B1�?�?    const patterns = [
        // 匹配"X�?格式，如"1�?, "2�?, "10�?
        /^(\d+)�?, 
        // 匹配"X�?格式，如"1�?, "2�?, "10�?
        /^(\d+)�?, 
        // 匹配"X-F", "X-FL", "X-L"等格式，�?1-F", "2-FL", "3-L"
        /^(\d+)-?(F|FL|L)/, 
        // 匹配"X#"格式，如"1#", "2#", "10#"
        /^(\d+)#/, 
        // 匹配"地下X�?, "地下X�?, "B1", "B2"等格�?        /^(地下(\d+)(楼|�?)|(B(\d+))/, 
        // 匹配"一�?, "二楼", "三楼"等中文数字格�?        /^(一|二|三|四|五|六|七|八|九|十|十一|十二)(楼|�?/
    ];
...
function parseExcel(file) {
    ...
    reader.onload = function(e) {
        ...
        // 检查表格格式：是列式格式（有明确列标题）还是行式格式（楼层+分组+成员列表�?        let isColumnFormat = false;
        const firstRow = range.s.r;
        
        // 检查第一行是否包含列标题，扩展可能的标题列表
        const possibleHeaders = ['姓名', '名字', '名称', '学生', '成员', '学号', 'ID', '编号', '工号', '楼层', '�?, '�?, '分组', '�?, '团队', '签到状况'];
        let headerCount = 0;
        ...
        for (let C = 0; C <= range.e.c; C++) {
            ...
            if (cell && cell.v) {
                ...
                // 支持模糊匹配（如"姓名"�?学生姓名"�?                if (possibleHeaders.some(header => cellValue.includes(header))) {
                    headerCount++;
                }
            }
        }
        
        // 如果找到2个或更多可能的列标题，并且包含姓名列，则认为是列式格�?        if (headerCount >= 2 && hasNameHeader) {
            isColumnFormat = true;
        } else {
        ...
        // 先尝试列式格式解�?        if (isColumnFormat) {
            // 列式格式解析
            // 先获取列标题的位�?            const headers = {};
            
            // 检查是否是特定的表头顺序：姓名，学号，楼层，分�?            const specificHeaders = ['姓名', '学号', '楼层', '分组'];
            let isSpecificFormat = true;
            
            // 检查前4列是否符合特定顺�?            for (let C = 0; C < Math.min(4, range.e.c + 1); C++) {
                const cell = firstSheet[XLSX.utils.encode_cell({r: firstRow, c: C})];
                if (cell && cell.v) {
                    ...
                    if (expectedHeader === '姓名' && !(header.includes('姓名') || header.includes('名字') || header.includes('名称') || header.includes('学生') || header.includes('成员'))) {
                        ...
                        isSpecificFormat = false;
                        break;
                    } else if (expectedHeader === '楼层' && !(header.includes('楼层') || header.includes('�?) || header.includes('�?))) {
                        isSpecificFormat = false;
                        break;
                    } else if (expectedHeader === '分组' && !(header.includes('分组') || header.includes('�?) || header.includes('团队'))) {
                        isSpecificFormat = false;
                        break;
                    }
                } else {
            ...
            if (isSpecificFormat) {
                // 如果是特定格式，直接使用�?列作为对应字�?                headers['姓名'] = 0;
                headers['学号'] = 1;
                headers['楼层'] = 2;
                headers['分组'] = 3;
            } else {
                // 否则使用原来的模糊匹配方�?                for (let C = 0; C <= range.e.c; C++) {
                    const cell = firstSheet[XLSX.utils.encode_cell({r: firstRow, c: C})];
                    if (cell && cell.v) {
                        const header = String(cell.v).trim();
                        // 支持模糊匹配，增加对更多姓名相关标题的识�?                        if (header.includes('姓名') || header.includes('名字') || header.includes('名称') || header.includes('学生') || header.includes('成员')) {
                            headers['姓名'] = C;
                        }
                        if (header.includes('学号') || header.includes('ID') || header.includes('编号') || header.includes('工号')) {
                            headers['学号'] = C;
                        }
                        if (header.includes('楼层') || header.includes('�?) || header.includes('�?)) {
                            headers['楼层'] = C;
                        }
                        if (header.includes('分组') || header.includes('�?) || header.includes('团队')) {
                            headers['分组'] = C;
                        }
                    }
                }
            ...
            // 确保姓名列存�?            if (headers['姓名'] !== undefined) {
                // 遍历数据行（从第二行开始）
                for (let R = firstRow + 1; R <= range.e.r; R++) {
                    ...
                    if (nameCell && nameCell.v) {
                        ...
                        if (name) {
                            ...
                            // 如果没有楼层信息，尝试从分组名称中提�?                            if (!floor && group) {
                ...
                // 如果没有找到姓名列，重置成员列表并尝试行式格式解�?                members.length = 0;
                isColumnFormat = false;
            }
        }
        ...
        if (!isColumnFormat || members.length === 0) {
            ...
            // 遍历每一�?            for (let R = range.s.r; R <= range.e.r; R++) {
                ...
                // 检查是否是有效的楼�?分组�?                if (groupCell && groupCell.v) {
                    ...
                    // 从第三列开始提取成员姓�?                    for (let C = 2; C <= range.e.c; C++) {
                        const nameCell = firstSheet[XLSX.utils.encode_cell({r: R, c: C})];
                        if (nameCell && nameCell.v) {
                            const name = String(nameCell.v).trim();
                            if (name) { // 确保姓名不为�?                                // 生成临时ID
...
function addMembersFromData(membersData) {
    // 检查权限：只有管理员才能添加成�?    if (currentRole !== 'admin') {
        alert('只有管理员才能添加成�?);
        return;
    }
    ...
    // 处理分组信息，确保所有分组存�?    const existingGroups = JSON.parse(localStorage.getItem('groups') || '[]');
    const existingGroupNames = new Set(existingGroups.map(g => g.name));
    const newGroups = [];
    
    newMembers.forEach(member => {
        if (member.group && member.group !== '' && !existingGroupNames.has(member.group)) {
            // 从分组名称中提取楼层信息（如果有的话�?        let floor = extractFloorFromGroupName(member.group);
        // 如果提取失败，使用默认楼�?        if (!floor) {
            floor = '1'; // 默认楼层
        }
...
async function exportData() {
    ...
    try {
        ...
        if (groupsError) {
            console.error('获取分组数据时出�?', groupsError);
            alert('获取数据失败，请稍后重试');
            return;
        }
        
        // 从Supabase获取成员数据
        let { data: members, error: membersError } = await window.appSupabase
            .from('members')
            .select('*');
        
        if (membersError) {
            console.error('获取成员数据时出�?', membersError);
            alert('获取数据失败，请稍后重试');
            return;
        }
        ...
        // 应用筛选条�?        const filterFloor = document.getElementById('statistics-filter-floor').value;
        const filterGroup = document.getElementById('statistics-filter-group').value;
        
        if (filterFloor) {
            // 先获取该楼层的所有分�?            const floorGroups = groups.filter(group => group.floor === filterFloor).map(group => group.name);
            // 过滤成员：属于该楼层的分�?            members = members.filter(member => member.group && floorGroups.includes(member.group));
        }
        ...
        if (attendanceError) {
            console.error('获取签到数据时出�?', attendanceError);
            alert('获取数据失败，请稍后重试');
            return;
        }
        
        // 将签到记录转换为与原代码兼容的格式（date -> memberId -> status�?        const attendance = {};
        attendanceRecords.forEach(record => {
        ...
        // 辅助函数：根据分组获取楼�?        function getGroupFloor(groupName, groups) {
            if (!groupName || !groups) return '';
            const group = groups.find(g => g.name === groupName);
            return group ? group.floor : '';
        }
        
        let csvContent = '姓名,学号/ID,总课�?;
        ...
        for (const member of members) {
            // 计算总课�?            const hours = await calculateMemberHours(member.id);
...
async function generateSummary() {
    ...
    try {
        ...
        // 将签到记录转换为与原代码兼容的格式（memberId -> status�?        const todayAttendance = {};
        todayAttendanceRecords.forEach(record => {
            todayAttendance[record.member_id] = record.status;
        });
        ...
        // 生成成员列表HTML，每5个成员换�?        function generateMemberList(membersList) {
            if (!membersList.length) return '<p>�?/p>';
...
function parseAttendanceExcel(file) {
    ...
    reader.onload = function(e) {
        ...
        const attendanceData = jsonData.map(row => {
            ...
            const status = row['签到状�?] || row['状�?] || row['status'] || row[Object.keys(row)[2]];
            return { 
...
function importAttendanceData(attendanceData) {
    ...
    attendanceData.forEach(item => {
        // 查找对应的成�?        const member = members.find(m => m.id === item.id || m.name === item.name);
        
        if (member) {
            // 标准化状�?            let normalizedStatus = 'absent';
...
async function deleteAllMembers() {
    ...
    try {
        // 删除所有签到记�?        const { error: attendanceError } = await window.appSupabase
            .from('attendance')
            .delete();
        
        if (attendanceError) {
            console.error('删除签到记录时出�?', attendanceError);
            alert('删除签到记录失败，请稍后重试');
            return;
        }
        
        // 删除所有成�?        const { error: membersError } = await window.appSupabase
            .from('members')
            .delete();
        
        if (membersError) {
            console.error('删除成员时出�?', membersError);
            alert('删除成员失败，请稍后重试');
            return;
        }
