...
// 添加错误处理，确保即使初始化失败也不会导致应用崩溃
try {
    // 使用正确的Supabase客户端创建方式
    window.appSupabase = createClient(supabaseUrl, supabaseKey);
    console.log('Supabase客户端创建成功:', window.appSupabase);
} catch (error) {
...
async function initData() {
    try {
        ...
        console.log('Supabase客户端配置:', {
            url: window.appSupabase?.url,
            key: window.appSupabase?.key ? '已配置' : '未配置'
        });
        
        // 尝试一个简单的请求来测试连接
        console.log('测试Supabase连接...');
        
        // 测试一个简单的数据库操作，验证连接是否正常
        try {
            const { data: testData, error: testError } = await window.appSupabase
                .from('members')
                .select('id', 'name')
                .limit(1);
            ...
            if (testError) {
                ...
                console.log('✓ Supabase连接测试成功，返回数据:', testData);
            }
        } catch (testConnectionError) {
            console.error('Supabase连接测试异常:', testConnectionError);
        }
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    // 检查Supabase客户端是否可用
    console.log('1. 检查Supabase createClient是否可用:', typeof window.createClient === 'function');
...
async function deleteAllGroups() {
    ...
    try {
        // 删除所有分组
        const { error: deleteGroupsError } = await window.appSupabase
            .from('groups')
            .delete();
...
async function updateGroupLeaderDropdown() {
    try {
        // 从Supabase获取负责人列表
        const { data: leaders, error: leadersError } = await window.appSupabase
            .from('users')
            .select('*')
            .eq('role', 'leader');
...
async function handleLogin() {
    ...
    try {
        ...
        // 检查Supabase客户端是否存在
        console.log('Supabase客户端:', window.appSupabase);
        ...
        if (role === 'member') {
            ...
            console.log('成员查询结果:', { members, memberError });
            
            if (memberError || !members) {
                console.log('成员验证失败:', memberError);
                if (errorDiv) {
            ...
            // 查询管理员或负责人信息
            const { data: users, error: userError } = await window.appSupabase
                .from('users')
                .select('*')
                .eq('username', username)
                .single();
            
            console.log('管理员/负责人查询结果:', { users, userError });
            
            if (userError || !users || users.password !== password || users.role !== role) {
                console.log('管理员/负责人验证失败:', { userError, users });
                if (errorDiv) {
...
async function deleteMember(memberId) {
    ...
    if (confirm('确定要删除该成员吗？')) {
        try {
            // 从Supabase中删除成员
            const { error: deleteMemberError } = await window.appSupabase
                .from('members')
                .delete()
                .eq('id', memberId);
...
async function loadAttendanceData() {
    try {
        ...
        // 将签到记录转换为与原代码兼容的格式        const attendanceMap = {};
...
async function deleteAllMembers() {
    ...
    try {
        // 删除所有签到记录
        const { error: attendanceError } = await window.appSupabase
            .from('attendance')
            .delete();
        ...
        // 删除所有成员
        const { error: membersError } = await window.appSupabase
            .from('members')
            .delete();
